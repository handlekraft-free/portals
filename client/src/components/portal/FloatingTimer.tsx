...
export function FloatingTimer() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 md:left-56">
      <div className="bg-[#0F172A]/95 backdrop-blur border-t border-white/10 px-4 py-2.5 flex items-center gap-3">
        ...
        {runningTimer ? (
          <>
            <p className="text-[#2563EB] font-mono font-bold text-base tabular-nums shrink-0" data-testid="text-timer-running">
              {fmt(timerSeconds)}
            </p>
          ...
          <>
            <input
              ...
              className="flex-1 bg-white/10 text-white placeholder-white/30 text-sm px-3 py-1.5 rounded-lg border border-white/10 focus:outline-none focus:border-[#2563EB]/60 focus:bg-white/15 transition min-w-0"
            />
            <button
              onClick={startTimer}
              disabled={!timerTask.trim()}
              data-testid="button-start-timer"
              className="flex items-center gap-1.5 bg-[#2563EB] hover:bg-[#2563EB]/80 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors shrink-0"
            >
