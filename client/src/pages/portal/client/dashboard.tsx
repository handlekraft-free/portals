...
function ClientDashboardContent() {
  ...
  return (
    <div>
      {/* Welcome */}
      <div className="bg-gradient-to-r from-[#0F172A] to-[#2563EB] rounded-2xl p-6 mb-6 text-white">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { icon: <Ticket className="w-5 h-5 text-[#2563EB]" />, label: "Open Tickets", value: loading ? "—" : stats.tickets, bg: "bg-teal-50" },
          { icon: <MessageSquare className="w-5 h-5 text-[#10B981]" />, label: "Unread Messages", value: loading ? "—" : stats.unread, bg: "bg-amber-50" },
          ...
          <Card key={i} className="border-0 shadow-sm">
            <CardContent className="pt-4 pb-3">
              ...
              <p className="text-2xl font-bold text-[#0F172A]" data-testid={`stat-${i}`}>{s.value}</p>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <a href="/portal/client/files" className="bg-[#2563EB] text-white rounded-xl p-4 flex flex-col items-center gap-2 text-sm font-medium hover:opacity-90 transition-opacity text-center" data-testid="link-upload">
          <Upload className="w-5 h-5" /> Upload File
        </a>
        <a href="/portal/client/messages" className="bg-[#0F172A] text-white rounded-xl p-4 flex flex-col items-center gap-2 text-sm font-medium hover:opacity-90 transition-opacity text-center" data-testid="link-message">
          <MessageSquare className="w-5 h-5" /> Send Message
        </a>
        <a href="/portal/client/tickets" className="bg-[#10B981] text-[#0F172A] rounded-xl p-4 flex flex-col items-center gap-2 text-sm font-medium hover:opacity-90 transition-opacity text-center" data-testid="link-ticket">
      ...
      <Card className="border-0 shadow-sm">
        <CardContent className="pt-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-[#0F172A]">Recent Support Tickets</h3>
            <a href="/portal/client/tickets" className="text-xs text-[#2563EB] hover:underline flex items-center gap-1">View all <ArrowRight className="w-3 h-3" /></a>
          </div>
