...
function MessagesContent() {
  ...
  if (activeThread) return (
    <div>
      ...
      <h2 className="text-lg font-display text-[#1A1F2B] mb-4">{activeThread.subject}</h2>
      ...
      <form onSubmit={sendReply} className="flex gap-2">
        <textarea value={reply} onChange={e => setReply(e.target.value)} placeholder="Type your reply..." rows={2} className="flex-1 border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0D7377]/30" data-testid="input-reply" />
        <Button type="submit" className="bg-[#0D7377] text-white self-end gap-1" data-testid="button-send-reply"><Send className="w-4 h-4" /> Send</Button>
      </form>
    </div>
  );
        ...
        <Card className="mb-4 border-[#0D7377]/20 shadow-sm">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between mb-3">
              <p className="font-semibold text-sm text-[#1A1F2B]">New Message</p>
            ...
            <form onSubmit={sendCompose} className="space-y-3">
              <input required value={compose.subject} onChange={e => setCompose(f => ({ ...f, subject: e.target.value }))} placeholder="Subject" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0D7377]/30" data-testid="input-subject" />
              <textarea required value={compose.body} onChange={e => setCompose(f => ({ ...f, body: e.target.value }))} placeholder="Your message..." rows={4} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0D7377]/30" data-testid="input-message-body" />
              <Button type="submit" className="bg-[#0D7377] text-white gap-1" data-testid="button-send-message"><Send className="w-4 h-4" /> Send</Button>
            </form>
          </CardContent>
        </Card>
      )}
      {loading ? <div className="space-y-2">{[...Array(3)].map((_, i) => <div key={i} className="h-16 bg-white rounded-xl animate-pulse" />)}</div> : (
        <div className="space-y-2">
          {messages.length === 0 ? <div className="text-center py-16 text-slate-400"><MessageSquare className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>No messages yet. Send your first message above.</p></div> :
            messages.map((m: any) => (
              <Card key={m.id} className={`border-0 shadow-sm cursor-pointer hover:shadow-md transition-shadow ${!m.readAt ? "border-l-4 border-l-[#0D7377]" : ""}`} onClick={() => loadThread(m.id)} data-testid={`msg-row-${m.id}`}>
                <CardContent className="py-3 px-4 flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm truncate ${!m.readAt ? "font-semibold text-[#1A1F2B]" : "text-slate-700"}`}>{m.subject}</p>
