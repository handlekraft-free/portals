...
- **Crew Bond**: `portal_users.crew_bond` integer counter (NOT XP, NOT a stat track). When a reviewer transitions a card from In Review → Done AND assignee≠reviewer, both bumped by +1; idempotent via `xp_events` rows with sourceTypes `crew_bond_review_a` / `crew_bond_review_b` (amount=0, stat=null) so global UNIQUE(source_type, source_id) lets exactly one row per side land per card. Server returns `crewBonds: CrewBond[]`; `apiRequest` dispatches `crew:bond` event; `CrewBondToaster` (mounted in EmployeeLayout) shows "⚓ Crew Bond — You shipped a review with X."
...
- **APIs**: `GET /api/crew/weekly` → `{ weekKey, questsShipped, reviewsCompleted, crewBondsThisWeek, threshold, fullCrew }`. `GET /api/crew/saga` → `{ weekKey, narrative, optOut, available }`. `PATCH /api/crew/saga/optout` (admin/canApprove only).
