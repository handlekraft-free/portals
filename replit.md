...
**handləkraft** is a full-stack web application for a 501(c)(3) nonprofit that builds free, open-source AI tools for community organizations and sustains the work through paid implementation services. The org concurrently trains non-traditional product builders by having them contribute to those same public projects. Public site messaging (May 2026 strategic proposal) leads with the open-source-plus-services operating model. Tools currently shipped: Board Portal, Longship Factory (ADHD/neurodivergent task manager). The platform features a scroll-based landing page, public application forms for fellowships and client requests, an admin dashboard for managing application queues, and a comprehensive internal portal system encompassing Employee, Client, Student, and Board Member portals. The brand name "handləkraft" (Norwegian for "the power to act") is consistently rendered with the Unicode schwa in all UI elements.
