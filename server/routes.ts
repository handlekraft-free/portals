  ...
  try {
    ...
    if (Number(eoCount) === 0) {
      await db.insert(employeeOnboardingItems).values([
        ...
        { title: "Bret Victor — Inventing on Principle (Video)", description: "A foundational talk about doing work that matters. Victor's framing — that good work flows from a principle you genuinely care about — is a useful north star for mission-driven teams. Especially valuable for non-traditional learners who may not have heard the field talk this way before.", linkUrl: "https://vimeo.com/36579366", section: "Part 3: Doing the Work Well", estimatedTime: "Watch ~50 min", roleFilter: "all", position: 8 },
    ...
    if (Number(meetingCount) === 0) {
      ...
      if (adminRows.length > 0) {
        ...
        if (m4?.id) {
          ...
          await db.insert(boardMeetingNotices).values({ meetingId: m4.id, method: "email", recipientCount: boardRows.length, sentBy: adminId, notes: "Sent via portal" });
        }
        const now2 = new Date();
